...
describe('components / Scale / ScaleTransition', () => {
  ...
  it('should apply correct CSS variables', () => {
    ...
    expect(style).toContain('--noc-transition-enter-duration: 280ms')
    expect(style).toContain('--noc-transition-leave-duration: 380ms')
    expect(style).toContain('--noc-transition-enter-delay: 60ms')
    expect(style).toContain('--noc-transition-leave-delay: 90ms')
    expect(style).toContain('--noc-transform-origin: bottom right')
  })
})
